...
let foodDB = (userData && userData.foodDB) || [
    ...
    { name: 'برنج', sodium: 5, potassium: 35, phosphorus: 40, water: 70 },    { name: 'سیب زمینی', sodium: 10, potassium: 400, phosphorus: 50, water: 80 },
...
let currentEditIndex = null;// تنظیمات نمودارها
const sodiumChart = new Chart(document.getElementById('sodiumChart'), {
    type: 'doughnut',
    data: {
        labels: ['مصرف شده', 'باقی مانده'], datasets: [{
            data: [0, 100],
...
const potassiumChart = new Chart(document.getElementById('potassiumChart'), {
    ...
    data: {
        labels: ['مصرف شده', 'باقی مانده'], datasets: [{
            data: [0, 100],            backgroundColor: ['rgba(54, 162, 235, 0.7)', 'rgba(220,220,220,0.7)'],
...
function updateChart() {
    ...
    document.getElementById('phosphorusPercentage').textContent = `${phosphorusPercentage.toFixed(0)}%`;    document.getElementById('waterPercentage').textContent = `${waterPercentage.toFixed(0)}%`;
...
function updateCurrentTotalsDisplay() {
    ...
    document.getElementById('currentTotals').innerHTML = `
        ...
        <p>پتاسیم: ${currentTotals.potassium} میلی گرم</p>
        <p>فسفر: ${currentTotals.phosphorus} میلی گرم</p>
...
function updateFoodSelect() {    const foodSelect = document.getElementById('foodSelect');
...
function addNewFood(event) {
    ...
    foodDB.push(newFood);
    userData = getData() || {};    userData.foodDB = foodDB;
    saveData(userData);
    updateEducationDB();
    updateFoodSelect();
...
function saveSettings(event) {
    ...
    settings.waterLimit = parseInt(document.getElementById('waterLimit').value);    userData = getData() || {};
