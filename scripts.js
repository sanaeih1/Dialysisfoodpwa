...
function loadSettings() {
  ...
  // اطمینان از مقداردهی اولیه حدود مواد مغذی
  limits = {
    sodium: userData.limits?.sodium || 2300,
    potassium: userData.limits?.potassium || 2500,
    phosphorus: userData.limits?.phosphorus || 900,
    protein: userData.limits?.protein || 80,
    water: userData.limits?.water || 1000
  };
...
function saveData() {
  try {
    ...
    userData.urineOutput = parseFloat(document.getElementById('urineOutput').value) || userData.urineOutput || null;
...
function updateDashboard() {
  ...
  totalElements.sodium.textContent = totals.sodium.toFixed(2);
  totalElements.potassium.textContent = totals.potassium.toFixed(2);
  totalElements.phosphorus.textContent = totals.phosphorus.toFixed(2);
  totalElements.protein.textContent = totals.protein.toFixed(2);
  totalElements.water.textContent = totals.water.toFixed(2);
  totalElements.kcal.textContent = totals.kcal.toFixed(2);

  // به‌روزرسانی حدود در داشبورد
  document.getElementById('sodiumLimitDisplayMg').textContent = limits.sodium.toFixed(2);
  document.getElementById('potassiumLimitDisplayMg').textContent = limits.potassium.toFixed(2);
  document.getElementById('phosphorusLimitDisplayMg').textContent = limits.phosphorus.toFixed(2);
  document.getElementById('proteinLimitDisplayMg').textContent = limits.protein.toFixed(2);
...
function sendSMS() {
  ...
  let message = `گزارش تغذیه بیمار (شناسه دستگاه: ${deviceId})\n`;
